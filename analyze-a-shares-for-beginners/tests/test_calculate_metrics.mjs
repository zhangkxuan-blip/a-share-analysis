...
import { calculateMetrics, discountedCashFlow, sma, rsi, weightedAverageCostOfCapital, weightedSentiment } from "../scripts/calculate_metrics.mjs";
...
const wacc = weightedAverageCostOfCapital({
  equityValue: 80, debtValue: 20, costOfEquity: 0.10, preTaxCostOfDebt: 0.05, taxRate: 0.25
});
assert.ok(Math.abs(wacc.value - 0.0875) < 1e-12);
const dcf = discountedCashFlow({
  fcffForecasts: [1, 1.1, 1.2], wacc: 0.10, terminalGrowth: 0.03,
  netDebt: 2, minorityInterests: 0.2, preferredEquity: 0, dilutedShares: 2
});
assert.equal(dcf.status, "ok");
assert.ok(dcf.value.enterpriseValue > dcf.value.presentValueForecasts);
assert.ok(dcf.value.perShare > 0);
assert.equal(discountedCashFlow({fcffForecasts: [1], wacc: 0.03, terminalGrowth: 0.03}).status, "not_meaningful");
